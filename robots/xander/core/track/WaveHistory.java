...
public class WaveHistory implements RoundBeginListener, GunListener, OpponentGunListener, TurnListener, BulletHitListener, Paintable {
	...
	public Wave createWave(Snapshot defenderSnapshot, Snapshot attackerSnapshot, double bulletPower, long bulletFiredTime, boolean opponentWave, boolean estimated) {
		...
		Wave wave = null;
		if (opponentWave) {
			wave = new Wave(defenderSnapshot, attackerSnapshot, bulletPower, bulletFiredTime, estimated);				
		} else {
	...
	public XBulletWave createXBulletWave(Snapshot defenderSnapshot, Snapshot attackerSnapshot,
		...
		XBulletWave wave = (XBulletWave) createWave(defenderSnapshot, attackerSnapshot, bullet.getPower(), bulletFiredTime, opponentWave, false);
	...
	public void opponentGunFired(OpponentGunFiredEvent event) {
		...
		Wave wave = createWave(event.getMySnapshot(), event.getOpponentSnapshot(), 
				adjustedFirePower, adjustedFireTime, true, event.isEstimated());
