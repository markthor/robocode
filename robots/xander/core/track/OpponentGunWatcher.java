...
public class OpponentGunWatcher implements RoundBeginListener, ScannedRobotListener, BulletHitListener, CollisionListener {
	...
	public void onScannedRobot(ScannedRobotEvent event) {
		...
		Long lastOpponentScanTime = opponentScanTimeMap.get(opponentName);
		opponentScanTimeMap.put(opponentName, Long.valueOf(event.getTime()));
		if (opponentEnergy == null || !opponentName.equals(lastScannedOpponent)) {
			...
			if (energyDrop != 0) {
				...
				if ((tslf >= tfc || collisionOnLastAssumedGunFire) 
					...
					OpponentGunFiredEvent ogfEvent = new OpponentGunFiredEvent(
							energyDrop, mySnapshot, snapshot, event.getTime(), 
							event.getTime()-lastOpponentScanTime.longValue() > 1);
