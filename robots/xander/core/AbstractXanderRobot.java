...
public abstract class AbstractXanderRobot extends AdvancedRobot {
	...
	public void run() {
		try {
			...
			while (true) {
				if (missedTurnSimulator != null && missedTurnSimulator.isTurnToBeMissed()) {
					log.info("Intentionally skipping turn " + getTime());
					onSkippedTurn(new SkippedTurnEvent(getTime()));
					execute();
					continue;
				}
