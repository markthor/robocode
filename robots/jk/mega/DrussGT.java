...
public class DrussGT extends AdvancedRobot{
   ...
   public void run(){
      ...
      if(shieldEnabled)
         try{shield = new EnergyDomeWorker(this);}
         catch(Exception ex){contain(ex);}
      ...
      while(true){
         ...
         // if(shieldEnabled)
            // System.out.println("Shield enabled at tick " + getTime() + ": " + shieldEnabled);
         execute();
      }
   }
   ...
   public void onScannedRobot(ScannedRobotEvent e){
      ...
      if(!shieldEnabled || TC){
         Bullet b = null;
         if(!MC)
            try{b = gun.onScannedRobot(e, dtg.futurePoint,dtg.futureHeading,dtg.futureVelocity,dtg.timeInFuture,dtg.enemyBP,TC);}
            catch(Exception ex){contain(ex);}
      
         if(!TC)
            try{move.logMyBullet(b);}
            catch(Exception ex){contain(ex);}
      }
   }
   public void onRobotDeath(RobotDeathEvent e){
      if(!TC) 
         try{move.onRobotDeath(e);}
         catch(Exception ex){contain(ex);}
      
      try{radar.onRobotDeath(e);}
      catch(Exception ex){contain(ex);}
   ...
   public void onHitByBullet(HitByBulletEvent e){
      if(!TC)
         try{move.onHitByBullet(e);}
         catch(Exception ex){contain(ex);}
      
      if(shieldEnabled)
         try{shield.onHitByBullet(e);}
         catch(Exception ex){contain(ex);}
   }      
   public void onHitRobot(HitRobotEvent e){
      if(!TC)
         try{move.onHitRobot(e);}
         catch(Exception ex){contain(ex);}
   }
   public void onBulletHitBullet(BulletHitBulletEvent e){
      ...
      if(!shieldEnabled)
         try{gun.onBulletHitBullet(e);}
         catch(Exception ex){contain(ex);}
      
      if(shieldEnabled)
         try{  shield.onBulletHitBullet(e);}
         catch(Exception ex){contain(ex);}
   }
   public void onBulletHit(BulletHitEvent e){
      ...
      if(!shieldEnabled)
         try{gun.onBulletHit(e);}
         catch(Exception ex){contain(ex);}
   
      if(shieldEnabled)
         try{  shield.onBulletHit(e);}
         catch(Exception ex){contain(ex);}
   }
